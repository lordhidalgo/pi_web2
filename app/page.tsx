...
const FAQItem = ({ question, answer, index, activeIndex, setActiveIndex }: FAQItemProps) => {
  const isOpen = activeIndex === index;
  const toggle = () => setActiveIndex(isOpen ? null : index);

  return (
    <div className={`rounded-2xl overflow-hidden shadow-[0_2px_8px_0_rgba(220,188,12,0.06)] transition-shadow duration-200 ${isOpen ? 'shadow-[0_4px_16px_0_rgba(220,188,12,0.13)] bg-gradient-to-br from-[#fffbe6] to-[#a7945c]' : 'bg-gradient-to-br from-[#fffbe6] to-[#ffe082]'}`}>
      <button
      ...
      >
        {question}
        <span className={`text-lg ml-3 transition-colors duration-200 ${isOpen ? 'text-white' : 'text-black'}`}>
          {isOpen ? "▲" : "▼"}
        </span>
      </button>
      {isOpen && <div className="p-4 sm:px-5 text-[#444] text-base animate-fadeInFaq">{answer}</div>}
    </div>
  );
};
  ...
  return (
    <>
      ...
      <div className="w-full mx-auto my-12 max-w-6xl bg-gradient-to-br from-[#fffbe6] to-[#ffe082] rounded-[32px] shadow-lg shadow-gray-400/30 p-10 relative z-10">
        ...
        <div className="flex flex-col md:flex-row justify-center md:justify-between gap-1 md:gap-2 mt-6 items-center md:items-stretch">
          <div className="flex flex-col gap-1 md:gap-2 items-center md:items-stretch">
            <div className="bg-white rounded-[18px] shadow-md shadow-gray-400/20 p-6 md:p-8 transform scale-70 transition-transform duration-300 hover:translate-y-[-8px] hover:scale-100 hover:shadow-xl hover:shadow-gray-400/30 w-full md:w-auto">
              <h4 className="text-xl font-bold text-[#9c7905] tracking-wider mb-2">Apoyo a artistas emergentes</h4>
              <p className="text-[#444] text-lg leading-relaxed text-center md:text-left">
